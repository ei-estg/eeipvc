...
export class GuildExtension<T> extends BaseExtension<T> {
    ...
    private async _memberJoined(member: any) {
        ...
        let roleRequestChannel = await member.guild.channels.fetch(
            '779491420079259659',
        )
        roleRequestChannel = roleRequestChannel.toString()

        await channel.send(
            `${string}Boas ${member.user}. Dá uma olhadela nas salas ${welcomeChannel} e ${roleRequestChannel} para ficares a conhecer as regras e ainda acederes a diferentes áreas do servidor.`,
        )
    }
